...
object Day08 extends Solution {
  ...
  def run(): Unit = {
    println("Year 2016, Day 8")
    val input = using("2016/day08.txt")(parseInput)
  ...
  override protected[year2016] def part1(input: List[String]): Int        = solution(input)._1
  override protected[year2016] def part2(input: List[String]): String     = solution(input)._2

  // Use a lazy val so we only have to perform the algorithm once and parts 1 and 2 can share the output
  private lazy val solution: List[String] => (Int, String) = input => {
