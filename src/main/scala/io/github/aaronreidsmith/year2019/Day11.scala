...
object Day11 extends Solution(2019, 11) {
  type I  = IntCode
  type O1 = Int
  type O2 = String

  override protected[year2019] def parseInput(file: Source): IntCode = IntCode(file)
  override protected[year2019] def part1(input: IntCode): Int        = paintHull(input).size
  override protected[year2019] def part2(input: IntCode): String = {
