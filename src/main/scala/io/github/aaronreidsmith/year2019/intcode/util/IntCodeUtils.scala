...
trait IntCodeUtils {
  protected implicit class SourceOps(file: Source) {
    def toInstructions: Instructions = {
      file.mkString.trim.split(',').zipWithIndex.map { case (value, index) => index.toLong -> value.toLong }.toMap
    }
  }
