...
object Day13 extends Solution {
  ...
  case class Packet(value: ujson.Value) extends Ordered[Packet] {
  ...
  override def parseInput(file: Source): List[Packet] = file.getLines().toList.collect {
    case line if line.nonEmpty => Packet(ujson.read(line))
  }

  override def part1(input: List[Packet]): Int = input.grouped(2).zipWithIndex.foldLeft(0) {
    case (acc, (Seq(a, b), index)) if a.compare(b) < 0 => acc + index + 1
    case (acc, _)                                      => acc
  }

  override def part2(input: List[Packet]): Int = {
