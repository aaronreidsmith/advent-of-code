...
object Day20 extends Solution(2017, 20) {
  ...
  private[year2017] case class Position(x: Int, y: Int, z: Int) extends Delta {
  ...
  private[year2017] case class Velocity(x: Int, y: Int, z: Int) extends Delta {
    def increment(xDelta: Int, yDelta: Int, zDelta: Int): Velocity = Velocity(x + xDelta, y + yDelta, z + zDelta)
  }
  private[year2017] case class Acceleration(x: Int, y: Int, z: Int)
  private[year2017] case class Particle(id: Int, position: Position, velocity: Velocity, acceleration: Acceleration) {
    def next: Particle = {
  ...
  override protected[year2017] def parseInput(file: Source): List[Particle] = {
  ...
  override protected[year2017] def part1(input: List[Particle]): Int = {
    @tailrec
  ...
  override protected[year2017] def part2(input: List[Particle]): Int = {
    @tailrec
